...
def get_colours():
    ...
    :return: list of colours which where each list item has the colour name, associated score and foreground colour text
    """
...
class StartGame:
    ...
    def check_rounds(self):
        ...
        try:
            ...
            if rounds_wanted > 0:
                #clear entry box and reset instructions label so that when users play a new game, they don't see an error
                self.num_rounds_entry.delete(0,END)
                self.choose_label.config(text="How many rounds do you want to play?")
                # invoke play class (and take number from each round)
                Play(rounds_wanted)
                # hide root window
                root.withdraw()
...
class Play:
    ...
    def __init__(self, how_many):
        ...
        # if the user press the 'x' on the game window, end the entire game!
        self.play_box.protocol('WM_DELETE_WINDOW', root.destroy)

        # body_font for most labels...
        body_font = ("Arial",12)
    ...
    def round_results(self, user_choice):
        """
        Retrieves which button was pushed (index 0 - 3), retrieves score and then compares it with median,
         updates and adds results to stats list."""
    ...
    def to_hints(self):
        """
        Displays hints for playing game. prevents users from aceessing dialogues
        that could lead to the program crashing
        """
        #checks we have played at least one round so that stat button is not enabled in error
        rounds_played = self.round_played.get()
...
class DisplayHints:
    ...
    def __init__(self, partner, rounds_played):
        ...
        # disable help, stat and end game buttons to prevent users from leaving a dialogue open and then going
        # back to the rounds dialogue
